...
function SpawnVehicle(vehicle, pGarage, Fuel, body, engine, plate, gType, IsViewing, IsShared)
    ...
    if  ParkingSpot ~= nil then
        if not IsViewing then
            ...
            QBCore.Functions.TriggerCallback('arabcodingteam-garage:server:GetVehicleProperties', function(properties)
            ...
            if IsShared then
                TriggerServerEvent('arabcodingteam-garage:server:updateSharedVehState', 'Out', plate, pGarage)
            else
                TriggerServerEvent('arabcodingteam-garage:server:updateVehicleState', 0, plate, pGarage)
                if plate ~= nil then
                    OutsideVehicles[plate] = CurrentDisplayVehicle
                    TriggerServerEvent('arabcodingteam-garages:server:UpdateOutsideVehicles', OutsideVehicles)
                end
            end
                ...
                if Var.DepotGetDestroyedVeh then
                    ...
                    if Engine < 40 then
                        if spawnpoint ~= nil then
                            QBCore.Functions.TriggerCallback('arabcodingteam-garage:server:PayDepotPrice', function(paid)
                                if paid then
                                    if not IsModelInCdimage(model) then return end
                                    RequestModel(model)
                                    while not HasModelLoaded(model) do Citizen.Wait(10) end
                                    ...
                                    QBCore.Functions.TriggerCallback('arabcodingteam-garage:server:GetVehicleProperties', function(properties)
                                        ...
                                        SetEntityAsMissionEntity(veh, true, true)
                                        TriggerServerEvent('arabcodingteam-garage:server:updateVehicleState', 0, Data.plate, Data.garage)
